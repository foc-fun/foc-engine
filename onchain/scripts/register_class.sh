...
if [ -z "$FOC_REGISTRY_CONTRACT" ]; then
  echo "Usage: $0 <FOC_REGISTRY_CONTRACT> <CLASS_HASH>"
  exit 1
fi
...
NAME="ContractName"
NAME_UTF8_HEX=$(echo -n $NAME | xxd -p -c 1000)
VERSION="0.0.1"
VERSION_UTF8_HEX=$(echo -n $VERSION | xxd -p -c 1000)
CALLDATA=$(echo -n $CLASS_HASH 0x$NAME_UTF8_HEX 0x$VERSION_UTF8_HEX)

echo "sncast --accounts-file $DEVNET_ACCOUNT_FILE --account $DEVNET_ACCOUNT_NAME --wait --json invoke --contract-address $FOC_REGISTRY_CONTRACT --function register_class --calldata $CALLDATA --url $RPC_URL"
sncast --accounts-file $DEVNET_ACCOUNT_FILE --account $DEVNET_ACCOUNT_NAME --wait --json invoke --contract-address $FOC_REGISTRY_CONTRACT --function register_class --calldata $CALLDATA --url $RPC_URL
