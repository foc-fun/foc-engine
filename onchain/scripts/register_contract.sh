...
if [ -z "$FOC_REGISTRY_CONTRACT" ]; then
  echo "Usage: $0 <FOC_REGISTRY_CONTRACT> <CONTRACT_ADDRESS> <CLASS_HASH> <CONTRACT_NAME> <CONTRACT_VERSION>"
  exit 1
fi
...
CALLDATA=$(echo -n $CONTRACT_ADDRESS $CLASS_HASH $CONTRACT_NAME $CONTRACT_VERSION)
